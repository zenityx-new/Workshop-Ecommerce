...
/**
 * Reads the logged-in user's DB cart grouped by shop — shared by the cart
 * page and the checkout page so both see identical data (price/stock live,
 * inactive products/suspended shops filtered out).
 */
export async function getDbCartGroups(userId: string): Promise<ShopCartGroup[]> {

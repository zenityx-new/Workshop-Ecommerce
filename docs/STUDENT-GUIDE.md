...
## ทางลัด: ให้ Claude Code ช่วยติดตั้งให้ (แนะนำ)

หลัง clone แล้ว คุณไม่ต้องทำข้อ 2–6 ด้วยมือทั้งหมดก็ได้ — เปิด Claude Code ในโฟลเดอร์โปรเจกต์แล้ววาง prompt นี้:

```
อ่าน docs/STUDENT-GUIDE.md แล้วช่วยติดตั้งโปรเจกต์นี้ให้ฉันทีละขั้นตอน
ขั้นไหนที่ต้องให้ฉันทำเอง (เช่น สร้าง Supabase project, ก๊อปคีย์, ปิด confirm email)
ให้หยุดบอกฉันชัดๆ ว่าต้องทำอะไรที่ไหน แล้วรอจนฉันทำเสร็จค่อยไปต่อ
ส่วนที่รันคำสั่งได้ (npm install, migrations, seed admin, รันเว็บ) ให้ทำให้เลย
และช่วยเปิดเว็บตรวจให้ด้วยว่าขึ้นได้ไม่ error
```

**Claude Code จะทำให้อัตโนมัติ:** `npm install`, สร้าง/เติมไฟล์ `.env.local` (คุณเอาคีย์มาวาง), ติดตั้ง `psql` ถ้าจำเป็น, รัน migrations, สร้าง admin, รันเว็บ แล้วตรวจให้

**แต่มี 3 ขั้นที่ Claude Code ทำแทนไม่ได้ (จะหยุดรอคุณ):**
1. **สร้างบัญชี + Supabase project** — ต้องสมัคร/สร้างเอง (Claude Code ไม่สร้างบัญชีแทนใคร)
2. **คัดลอกคีย์** จากหน้า Supabase (Project URL, anon key, service_role key, connection string) มาให้มัน
3. **ปิด Confirm email** ใน Supabase (หน้าเว็บ)

> เหตุผลที่ทำแทนไม่ได้: Claude Code จะไม่สมัครบัญชี/กรอกรหัสผ่านเข้าเว็บแทนคุณ และเข้าหน้า dashboard Supabase ของคุณไม่ได้ — 3 ขั้นนี้จึงต้องเป็นคุณเสมอ (ใช้เวลารวมไม่กี่นาที) ที่เหลือปล่อยให้ Claude Code จัดการได้

อยากทำเองทีละขั้นเพื่อเข้าใจมากขึ้น? อ่านข้อ 3–6 ด้านล่างต่อได้เลย
...
### ปิด "Confirm email" ใน Supabase (จำเป็น — ทำครั้งเดียว)

ค่าเริ่มต้นของ Supabase คือ ผู้สมัครใหม่ต้องกดยืนยันลิงก์ในอีเมลก่อนถึงจะล็อกอินได้ ตอน dev/workshop เราปิดทิ้งเพื่อให้สมัครแล้วใช้งานได้ทันที:

1. เข้า Supabase project ของคุณ → เมนูซ้าย **Authentication**
2. ไปที่ **Sign In / Providers** (บางเวอร์ชันชื่อ **Providers**) → คลิกที่ **Email**
3. ปิดสวิตช์ **"Confirm email"** (ให้เป็นสีเทา/off)
4. กด **Save**

> หา "Confirm email" ไม่เจอ? ลองดูที่ **Authentication → Settings/Configuration** หรือช่องค้นหาในหน้า Auth พิมพ์ว่า `confirm` — ตำแหน่งเมนูของ Supabase ปรับเปลี่ยนเป็นระยะ แต่ชื่อ setting คือ "Confirm email" เสมอ
>
> ปิดแล้วทดสอบ: สมัครสมาชิกใหม่ในเว็บ ควรเข้า `/account` ได้ทันทีโดยไม่ต้องเปิดอีเมล
>
> **หมายเหตุ production:** ถ้านำขึ้นใช้งานจริงกับผู้ใช้จริง ควร**เปิด** Confirm email กลับ (กันคนสมัครด้วยอีเมลมั่ว) — การปิดไว้เหมาะกับตอนเรียน/ทดสอบเท่านั้น

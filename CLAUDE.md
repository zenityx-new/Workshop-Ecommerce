...
| 8 — Cron & Deploy | ✅ เสร็จ + **deploy ขึ้น Vercel production แล้ว** (2026-07-20) | — |
...
- **Deploy จริงขึ้น Vercel แล้ว** (2026-07-20) — repo GitHub: `zenityx-new/Workshop-Ecommerce` (push ประวัติครบทุก commit) · Vercel project `workshop-ecommerce` (scope `aekkarat-2883`, แผน **Hobby**) · โดเมน production: **https://workshop-ecommerce-phi.vercel.app** (ชื่อ `workshop-ecommerce.vercel.app` ถูกใช้แล้ว Vercel เติม `-phi`) · env 5 ตัวตั้งใน Vercel (production): SUPABASE URL/anon/service_role (ชี้ Supabase `pxzfnnupgsmgaqnsoxgb` เดิมที่เคลียร์ข้อมูลเหลือ admin+หมวดหมู่), `NEXT_PUBLIC_SITE_URL`=โดเมน prod, `CRON_SECRET`=สุ่มใหม่เฉพาะ prod (ไม่เก็บค่าในไฟล์/repo) · **Hobby จำกัด cron วันละครั้ง** จึงปรับ `vercel.json` เป็นรายวัน (auto-cancel 02:00, auto-complete 03:00) — เกณฑ์อายุ 24ชม./7วันอยู่ใน RPC เหมือนเดิม ถ้าอัป Pro ค่อยเพิ่มความถี่ · ทดสอบ prod ผ่าน: หน้าแรก 200, /api/cron/* = 401 เมื่อไม่มี secret และรันได้เมื่อใส่ secret, robots/sitemap ใช้โดเมนถูก · มีคู่มือนักเรียน `docs/STUDENT-GUIDE.md` (clone→ติดตั้ง→ใช้ Claude Code พัฒนาต่อ) · เครื่อง dev ติดตั้ง Vercel CLI + login เป็น `aekkarat-2883` แล้ว (deploy ซ้ำด้วย `vercel deploy --prod --yes`)
